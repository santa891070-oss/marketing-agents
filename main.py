...
# Инициализация LLM через OpenAI
...
async def run_agent(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await update.message.reply_text("⏳ Анализирую рынок и пишу пост...")
    
    task1 = Task(description=f"Проанализируй рынок по теме: {user_input}", expected_output='Отчет', agent=researcher)
